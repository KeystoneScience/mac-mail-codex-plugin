...
printf '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}\n{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}\n' \
  ...
  | "$PYTHON_BIN" -c 'import json,sys; lines=sys.stdin.read().splitlines(); init=json.loads(lines[0]); tools=json.loads(lines[1])["result"]["tools"]; assert init["result"]["serverInfo"]["version"] == "0.6.3"; assert len(tools) >= 22; print(f"JSON-RPC OK: {len(tools)} tools")'
